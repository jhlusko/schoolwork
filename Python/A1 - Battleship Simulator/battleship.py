...
def ship_length(ship_name):
    ''' (str) -> int
    Return length of ship ship_name'''
...
def addPlayer(player):
    '''(Player) -> NoneType
    Add Player object player to player_list'''
...
def addBoard(board):
    '''(Board) -> NoneType
    Add Board object board to board_list'''
...
class Player(object):
    ...
    def __init__(self, name, is_human):
        '''(Player, str, bool) -> NoneType
        Create a player object with a name and whether it's a\
        human/computer.'''
        self.name = name
...
class Ship(object):
    ...
    def __init__(self, name, locant, direction):
        ''' (Ship, str, tuple, str) -> NoneType
        Creates a ship object, with a name, head location, direction to \
        be placed away from head, a hitcount, size, and most importantly \
        a list of the positions the ship occupies.'''
...
class Board(object):
    ...
    def __init__(self, player, board_size):
        '''(Board, Player, int) -> NoneType
        Create a board of size board_size, which knowns who it belongs to,\
    ...
    def displayBoard(self):
        '''(Board) -> NoneType
        Print a list of lists containing each position and whether it's been\
    ...
    def removedS(self):
        '''(Board)-> NoneType
        Print the board (list of lists) with the 'S's replaced by '0's'''
    ...
    def enoughRoom(self, name, locant, direction):
        ...
        Determine if there is room to place the ship at the specified \
        location'''
    ...
    def addShip(self, name, locant, direction):
        '''(Board, str, tuple, str) ->  NoneType
        Update Board object and create new Ship object to reflect new ship.'''
        shipsize = ship_length(name)
        (x, y) = locant
    ...
    def randomlyAddShips(self, name, num_ships):
        '''(Board, str, int) -> NoneType
        Randomly place num_ships amount of name type ships, and update the \
    ...
    def move(self, x, y):
        ...
        Update the board to reflect a new move, and prints the outcome.'''
    ...
    def changeValue(self, new_value, x_value, y_value):
        '''(Board, str, int, int) -> NoneType or str
...
def badCoordinates(board):
    '''(Board)-> tuple
    Ask for coordinate input, and then format it into an appropriate \
    tuple.'''
